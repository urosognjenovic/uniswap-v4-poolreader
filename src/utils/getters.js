...
export const getPositionInfoWithPositionID = async(stateView, poolId, positionId) => {
  try {
    const [
      liquidity,
...
// Get the position info of a pool with a specific position ID
export const getPositionInfoWithPositionDetails = async(stateView, poolId, owner, tickLower, tickUpper, salt) => {
  try {
    const [
      liquidity,
      feeGrowthInside0LastX128,
      feeGrowthInside1LastX128
    ] = await stateView.read.getPositionInfo(poolId, owner, tickLower, tickUpper, salt);
